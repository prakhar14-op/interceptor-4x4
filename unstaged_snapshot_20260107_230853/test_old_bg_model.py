"""
TEST OLD BG MODEL ON 100 VIDEOS
Test the baseline_student.pt model (old BG model from archive)
"""
...
# ============================================================================
# OLD BG MODEL ARCHITECTURE (ResNet-based baseline)
# ============================================================================
class OldBGModel(nn.Module):
    """Reconstruct old baseline BG model architecture"""
    
    def __init__(self, num_classes=2):
        super().__init__()
        
        # Based on baseline_student.pt, likely uses ResNet backbone
...
# MODEL TESTING
# ============================================================================
def test_old_bg_model():
    """Test old BG model on 100 videos"""
    
    print("🔍 TESTING OLD BG MODEL (baseline_student.pt)")
    print("="*60)
    
    # Load old model
    model_path = Path("models/baseline_student.pt")
    
    if not model_path.exists():
        # Try alternative paths
        alt_paths = [
            Path("_archive/models/baseline_student.pt"),
            Path("baseline_student.pt")
        ]
        for alt_path in alt_paths:
            ...
            print(f"   - models/baseline_student.pt")
            print(f"   - _archive/models/baseline_student.pt")
            print(f"   - baseline_student.pt")
            return
    ...
    if not model_path.exists():
        ...
        print(f"   Please ensure baseline_student.pt is in _archive/models/")
        return
    ...
    # Create model
    model = OldBGModel(num_classes=2)
    ...
    results = {
        'model_info': {
            'file': 'baseline_student.pt',
            'location': '_archive/models/',
    ...
    with open('old_bg_model_test_results.json', 'w') as f:
        json.dump(results, f, indent=2)
    
    print(f"\n✅ Results saved to: old_bg_model_test_results.json")
