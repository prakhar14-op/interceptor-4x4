"""
TEST OLD AV MODEL ON 100 VIDEOS
Test the av_model_student.pt model (old AV model)
"""
...
# ============================================================================
class OldAVModel(nn.Module):
    """Reconstruct old baseline AV model architecture"""
...
# MODEL TESTING
# ============================================================================
def test_old_av_model():
    """Test old AV model on 100 videos"""
    
    print("🔍 TESTING OLD AV MODEL (av_model_student.pt)")
    print("="*60)
    
    # Load old model
    model_path = Path("models/av_model_student.pt")
    
    if not model_path.exists():
        # Try alternative paths
        alt_paths = [
            Path("_archive/models/av_model_student.pt"),
        ...
        for alt_path in alt_paths:
            ...
            print(f"   - models/av_model_student.pt")
            print(f"   - _archive/models/av_model_student.pt")
            print(f"   - av_model_student.pt")
            return
    ...
    # Create model
    model = OldAVModel(num_classes=2)
    ...
    with open('old_av_model_test_results.json', 'w') as f:
        json.dump(results, f, indent=2)
    
    print(f"\n✅ Results saved to: old_av_model_test_results.json")
