"""
TEST OLD CM MODEL ON 100 VIDEOS
Test the cm_model_student.pt model (old CM model)
"""
...
# ============================================================================
# OLD CM MODEL ARCHITECTURE (ResNet-based baseline)
# ============================================================================
class OldCMModel(nn.Module):
    """Reconstruct old baseline CM model architecture"""
...
# MODEL TESTING
# ============================================================================
def test_old_cm_model():
    """Test old CM model on 100 videos"""
    
    print("🔍 TESTING OLD CM MODEL (cm_model_student.pt)")
    print("="*60)
    
    # Load old model
    model_path = Path("models/cm_model_student.pt")
    
    if not model_path.exists():
        # Try alternative paths
        alt_paths = [
            Path("_archive/models/cm_model_student.pt"),
            Path("cm_model_student.pt")
        ]
        for alt_path in alt_paths:
            ...
            print(f"   - models/cm_model_student.pt")
            print(f"   - _archive/models/cm_model_student.pt")
            print(f"   - cm_model_student.pt")
            return
    ...
    # Create model
    model = OldCMModel(num_classes=2)
    ...
    with open('old_cm_model_test_results.json', 'w') as f:
        json.dump(results, f, indent=2)
    
    print(f"\n✅ Results saved to: old_cm_model_test_results.json")
